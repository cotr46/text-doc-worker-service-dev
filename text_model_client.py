...
{{"status": "positive|negative|neutral|unknown", "summary": "...", "details": [...], "sources": [...], "confidence": 0.85}}"""
        ...
        # Get prompt
        if analysis_type in prompts and entity_type in prompts[analysis_type]:
            prompt = prompts[analysis_type][entity_type]
        else:
...
Provide comprehensive analysis with JSON response including status, summary, details, sources, and confidence."""
    ...
    def _make_request(self, model_name: str, prompt: str, attempt: int, **kwargs) -> ModelResponse:
        ...
        response = requests.post(
            self.chat_endpoint, headers=headers, json=payload, timeout=self.timeout_seconds
        )
        ...
        return ModelResponse(
            content=content, model=model_name, usage=response_data.get("usage", {}),
            response_time=response_time, status_code=response.status_code,
            raw_response=response_data, sources=response_data.get("sources", [])
        )
...
{{"status": "positive|negative|neutral|unknown", "summary": "Brief summary of findings", "details": [{{"type": "position", "description": "Political position details"}}, {{"type": "connection", "description": "Family/associate connections"}}, {{"type": "risk", "description": "Risk assessment details"}}], "sources": ["source1", "source2"], "confidence": 0.85}}"""
            },
            "negative-news": {
...
Format the response as structured JSON with the following structure:
{{"status": "positive|negative|neutral|unknown", "summary": "Brief summary of findings", "details": [{{"type": "news", "description": "Negative news details"}}, {{"type": "issue", "description": "Type of issues identified"}}, {{"type": "risk", "description": "Risk assessment details"}}], "sources": ["source1", "source2"], "confidence": 0.85}}""",
...
Format the response as structured JSON with the following structure:
{{"status": "positive|negative|neutral|unknown", "summary": "Brief summary of findings", "details": [{{"type": "case", "description": "Legal case details"}}, {{"type": "outcome", "description": "Case outcomes"}}, {{"type": "risk", "description": "Risk assessment details"}}], "sources": ["source1", "source2"], "confidence": 0.85}}""",
...
{{"status": "positive|negative|neutral|unknown", "summary": "Brief summary of findings", "details": [{{"type": "case", "description": "Legal case details"}}, {{"type": "outcome", "description": "Case outcomes"}}, {{"type": "risk", "description": "Risk assessment details"}}], "sources": ["source1", "source2"], "confidence": 0.85}}"""
        ...
        if actual_type in prompt_templates and entity_type in prompt_templates[actual_type]:
            prompt = prompt_templates[actual_type][entity_type]
        else:
